...
use Games::Roguelike::Area::Rooms;
use Games::Roguelike::Area::Cavelike;
...
sub init {
    ...
    croak("need to specify a name for this area")
      if $opts{world} && !$opts{name};
...
sub genroom {
    ...
    for ( my $i = -1; $i <= $rw; ++$i ) {
        next if ( $rx + $i ) < 0;
        $m->[ $rx + $i ][ $ry - 1 ] = $self->{wsym}
          unless $m->[ $rx + $i ][ $ry - 1 ] || ( $ry == 0 );
        $m->[ $rx + $i ][ $ry + $rh ] = $self->{wsym}
          unless $m->[ $rx + $i ][ $ry + $rh ];
    }
    for ( my $i = 0; $i < $rw; ++$i ) {
        for ( my $j = 0; $j < $rh; ++$j ) {
            $m->[ $rx + $i ][ $ry + $j ] = $self->{fsym}
              unless $m->[ $rx + $i ][ $ry + $j ];
        }
    }
    for ( my $i = -1; $i <= $rh; ++$i ) {
        next if ( $ry + $i ) < 0;
        $m->[ $rx - 1 ][ $ry + $i ] = $self->{wsym}
          unless $m->[ $rx - 1 ][ $ry + $i ] || ( $rx == 0 );
        $m->[ $rx + $rw ][ $ry + $i ] = $self->{wsym}
          unless $m->[ $rx + $rw ][ $ry + $i ];
    }
    if ( $opts{door} ) {
        if ( rand() > .5 ) {
...
sub makepath {
    ...
    if ( $d =~ /^n|s$/ ) {
        $x = $mc[ $DI{w} ]->[0]
          + rand() * ( $mc[ $DI{e} ]->[0] - $mc[ $DI{w} ]->[0] );
    }
    else {
        $y = $mc[ $DI{n} ]->[1]
          + rand() * ( $mc[ $DI{s} ]->[1] - $mc[ $DI{n} ]->[1] );
    }
    intify( $x, $y );
...
sub draw {
    ...
    for ( my $y = $oy; $y < ( $disph + $oy ); ++$y ) {
        ...
        if ( $self->{debugmap} ) {
            $con->addstr( $y - $oy + $dispy, $debugx,
                sprintf( "%02.0d|", $y ) );
        }
        for ( my $x = $ox; $x < $dispw + $ox; ++$x ) {
            if ( my $memtyp
                = $self->checkmap( $vp, $x, $y, $self->{map}->[$x][$y] ) )
            {
                my ($color) = $self->{color}->[$x][$y];
                my $sym
                  = ( $memtyp == 2 )
                  ? $vp->{memory}->{ $self->{name} }->[$x][$y]
                  : $self->{map}->[$x][$y] ? $self->{map}->[$x][$y]
                  :                          ' ';
                $color = $self->{memcolor}
                  if $memtyp == 2
                  && $self->{memcolor}
                  ;    # if the area is memorized, then draw as gray
                $con->attrch( $color, $y - $oy + $dispy, $x - $ox + $dispx,
                    $sym );
            }
            else {
...
# $xoff, $yoff musy be ADDED to get absolute coords (relative to console box)
    ...
    if ( $self->checkpov( $vp, $ob->{x}, $ob->{y} ) ) {
        if (   ( ( $ob->{y} - $oy ) >= 0 )
            ...
            && ( ( $ob->{y} - $oy ) < $opts->{disph} ) )
        {
            $con->attrch( $ob->{color}, $ob->{y} - $oy + $yoff,
                $ob->{x} - $ox + $xoff, $ob->{sym} );
        }
        ...
        if (   $vp
            && $ob != $vp
            && !(
                $vp->{memory}->{ $self->{name} }->[ $ob->{x} ][ $ob->{y} ] eq
                $ob->{sym}
            )
          )
        {
            $vp->{memory}->{ $self->{name} }->[ $ob->{x} ][ $ob->{y} ]
              = $ob->{sym};
            $vp->{sawnew} = 1;
        }
        return 1;
...
sub checkpov {
    ...
    if ($OKINLINEPOV) {
        ...
        return checkpov_c( $vx, $vy, $x, $y, $self->{map}, $self->{noview},
            $self->{debugfov} ? 1 : 0 );
    }
    ...
    for ( my $i = 1; $i <= $dist; $i += 0.5 ) {
        ...
        for ( my $j = 0; $j < 4; ++$j ) {
            ...
            if ( int( $x[$j] ) eq $x && int( $y[$j] ) eq $y ) {
                print
                  "$i: sub $j: $x[$j],$y[$j] SAME ($self->{map}->[$x[$j]][$y[$j]])\n"
                  if $self->{debugfov};
                next;
            }
            if (   $dx != 0
                ...
                && ( abs( $dy / $dx ) > 0.1 ) )
            {
                ...
                {
                    print
                      "$i: sub $j: $x[$j],$y[$j] PEEK ($self->{map}->[$x[$j]][$y[$j]])\n"
                      if $self->{debugfov};
                    next;
                }
            }
            if ( ( $self->{map}->[ $x[$j] ][ $y[$j] ] =~ /^(#|\+)$/ ) ) {
                $ok[$j] = 0;
                print
                  "$i: sub $j: $x[$j],$y[$j] WALL ($self->{map}->[$x[$j]][$y[$j]])\n"
                  if $self->{debugfov};
            }
            else {
                print
                  "$i: sub $j: $x[$j],$y[$j] OK ($self->{map}->[$x[$j]][$y[$j]])\n"
                  if $self->{debugfov};
            }
        }
...
sub additem {
    ...
    if ( $item->setcont($self) ) {
        if ( !defined( $item->{x} ) ) {
            ( $item->{x}, $item->{y} )
              = $self->findrandmap( $self->{fsym}, 0, 1 );
        }
    }
