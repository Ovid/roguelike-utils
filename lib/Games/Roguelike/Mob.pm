...
sub new {
    my $pkg  = shift;
    my $area = shift;
    croak("can't create mob without area")
      unless $area && $area->isa('Games::Roguelike::Area');
    ...
    $self->{pov}    = -1;
    $self->{singleminded}
      = 0;    # whether it tries to wander around in pursuit of a goal
    ...
    if ( !defined( $self->{x} ) ) {
        ( $self->{x}, $self->{y} )
          = $area->findrandmap( $area->{fsym}, 0, 1 );
    }
...
sub autoex {
    ...
    if ( !$world && $self->{autopath} && @{ $self->{autopath} } ) {
        ...
        if ( $self->{memory}->{ $self->{area}->{name} }
            ->[ $self->{autocx} ][ $self->{autocy} ] )
        {
            $self->{autopath} = undef;
        }
        return $moved;
    ...
    while (@f) {
        ...
        for ( my $d = 0; $d < 8; ++$d ) {
            next
              unless $self->{memory}->{ $self->{area}->{name} }
              ->[ $c->[0] ][ $c->[1] ]
              ;              # has to be "moving from" a place we have seen
...
sub move {
    ...
    my $ny = $self->{y} + $DD{$d}->[1];
    my $r;
    $r = $self->checkmove( $nx, $ny, scalar $self->{area}->mobat( $nx, $ny ),
        $testonly );
